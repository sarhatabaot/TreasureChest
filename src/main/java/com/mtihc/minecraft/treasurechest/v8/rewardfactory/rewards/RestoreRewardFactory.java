...
public class RestoreRewardFactory extends RewardFactory implements Observer {
    ...
    void restore(Player player, RestoreReward reward) throws RewardException {
        ...
        Vector3 min = Vector3.at(reward.getMin().getBlockX(),reward.getMin().getBlockY(),reward.getMin().getBlockZ());
        Vector3 max = Vector3.at(reward.getMax().getBlockX(),reward.getMax().getBlockY(),reward.getMax().getBlockZ());
        String snapshot = reward.getSnapshotName();

        String id = getTaskId(worldName, reward.getMin(), reward.getMax(), snapshot);
        if (!queue.has(id)) {
